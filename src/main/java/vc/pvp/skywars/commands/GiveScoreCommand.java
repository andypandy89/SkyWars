...
@CommandDescription("Give score to a player.")
@CommandPermissions({ "skywars.command.givescore" })
public class GiveScoreCommand implements CommandExecutor {

	public boolean onCommand(CommandSender sender, Command command,
			String label, String[] args) {
		if (args.length <= 2) {
			sender.sendMessage(new Messaging.MessageFormatter().setVariable(
					"example", "/sw give player score").format(
			...
			if (this.isInteger(args[2])) {
				...
				gp.addScore(Integer.parseInt(args[2]));
